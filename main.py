...
def get_next_run_time(hour, minute, days):
...
async def send_poll_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    now = datetime.datetime.now(ZoneInfo("Europe/Moscow"))
    tomorrow = now + datetime.timedelta(days=1)  # <-- Ключевое изменение
    ...
    next_run = get_next_run_time(hour, minute, poll_data['days'])
    
    if next_run <= tomorrow + datetime.timedelta(days=7):
        next_run += datetime.timedelta(weeks=1)
    
    context.job_queue.run_once(send_poll_job, when=next_run, data=poll_data)
    logger.info(f"Опрос перепланирован на: {next_run.strftime('%Y-%m-%d %H:%M')}")

def schedule_polls(application, polls):
    for poll in polls:
        try:
            hour, minute = map(int, poll['time'].split(':'))
            next_run = get_next_run_time(hour, minute, poll['days'])
            application.job_queue.run_once(send_poll_job, when=next_run, data=poll)
...
def main():
    ...
    if config and 'polls' in config:
        schedule_polls(application, config['polls'])
    else:
